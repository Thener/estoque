	...
	$.addFlex=function(t,p){
        ...
        var g={
            ...
            fixHeight:function(newH){
                ...
                if(nd>newH)$(g.nDiv).height(newH).width(200);else $(g.nDiv).height('auto').width('auto');
                $(g.block).css({
                    height:newH,
                    marginBottom:(newH*-1)
                    });
...
dragEnd:function(){
    if(this.colresize){
        ...
        $(this.colCopy).remove();
        if(this.dcolt!=null){
            if(this.dcoln>this.dcolt)$('th:eq('+this.dcolt+')',this.hDiv).before(this.dcol);else $('th:eq('+this.dcolt+')',this.hDiv).after(this.dcol);
...
switchCol:function(cdrag,cdrop){
    $('tbody tr',t).each(function(){
        if(cdrag>cdrop)$('td:eq('+cdrop+')',this).before($('td:eq('+cdrag+')',this));else $('td:eq('+cdrop+')',this).after($('td:eq('+cdrag+')',this))
            });
    if(cdrag>cdrop){
        ...
        $.each(data.rows,function(i,row){
            ...
            $('thead tr:first th',g.hDiv).each(function(){
                ...
                if (selec == 0) {
                	...
                	selec = 1;
                	if (p.checkFirstColumn) { //Pedro
                		td.innerHTML = '<input style="margin-right:10px" type="checkbox" id="' + row.cell[idx] + '" value="' + row.cell[idx] + '" />' + row.cell[idx]; // Pedro
                	} else { //Pedro
                		td.innerHTML = row.cell[idx]; // Pedro
                	}
...
addRowProp:function(){
    $('tbody tr',g.bDiv).each(function(){
        $(this).click(function(e){
            ...
            $(this).addClass('trSelected'); //$(this).toggleClass('trSelected');
...
if(p.striped){
    ...
    for(var s=0;s<sitems.length;s++){
        ...
        $(g.sDiv).append("<div class='sDiv2'>"+p.findtext+" <input type='text' value='"+p.query+"' size='30' name='q' class='qsbox' /> "+" <select name='qtype'>"+sopt+"</select>  <input type='button' name='pesquisar' class='pPesquisar' onclick='javascript:void(0);' value='Pesquisar' />  </div>");
...
$('input[name=pesquisar]',g.sDiv).click(function(){
        g.doSearch()
    });
...
$.fn.noSelect=function(p){
    ...
    if(prevent){
        ...
        return this.each(function(){
            ...
            else if($.browser.opera)$(this).unbind('mousedown');else $(this).removeAttr('unselectable','on')
                })
        }
    }
})(jQuery);